...
import { CheckIcon, CaretDownIcon as ChevronDown, XIcon, MagicWandIcon as WandSparkles } from "@phosphor-icons/react";
 ...
 */
const multiSelectVariants = cva("ml-0.5 transition-all duration-300 ease-in-out", {
  variants: {
    variant: {
      default: "hover:border-muted-foreground text-foreground bg-card hover:bg-background/60",
...
interface MultiSelectProps extends Omit<React.ButtonHTMLAttributes<HTMLButtonElement>, "animationConfig">, VariantProps<typeof multiSelectVariants> {
  ...
  /** Error state */
  error?: boolean;
  /** Error message shown below component */
  errorMessage?: string | React.ReactNode;

  /** Optional container for the Popover Portal */
  ...
  (
    ...
    return (
      <>
        ...
        <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen} modal={modalPopover}>
          ...
          <PopoverTrigger asChild>
            <Button
              ...
              className={cn(
                "flex p-1 rounded-md border min-h-10 h-auto items-center justify-between bg-inherit hover:bg-background/60 hover:border-muted-foreground [&_svg]:pointer-events-auto",
                autoSize ? "w-auto" : "w-full",
                responsiveSettings.compactMode && "min-h-8 text-sm",
                screenSize === "mobile" && "min-h-12 text-base",
                disabled && "opacity-50 cursor-not-allowed [&_svg]:pointer-events-none",
                className
              )}
              style={{
                ...widthConstraints,
                ...
                <div className="flex justify-between items-center w-full">
                  ...
                  >
                        ...
                        return (
                          <Badge
                            ...
                            className={cn(
                              ...
                              "[&>svg]:pointer-events-auto font-normal"
                            )}
                            style={{
                              ...badgeStyle,
                    ...
                    {selectedValues.length > responsiveSettings.maxCount && (
                      <Badge
                        className={cn(
                          "bg-transparent font-normal text-foreground border-foreground/1 hover:bg-transparent",
                          getBadgeAnimationClass(),
                          multiSelectVariants({ variant }),
                          responsiveSettings.compactMode && "text-xs px-1.5 py-0.5",
                          singleLine && "flex-shrink-0 whitespace-nowrap",
        ...
        {error && errorMessage && <p className="mt-1 text-sm text-destructive leading-tight">{errorMessage}</p>}
      </>
    );
  }
);
