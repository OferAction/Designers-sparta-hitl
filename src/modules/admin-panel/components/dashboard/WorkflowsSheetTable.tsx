...
import { UserPerWorkflow, WorkflowDataRow } from "@/types/user";

import AdminPageTable from "./AdminPageTable";
import { ProjectUserColumns, projectUserGlobalFilterFn } from "./columns";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { useGetEntityUsers, useUpdateUserInWorkflowsPermissionsBatch } from "@/services/securityService";

export const WorkflowsSheetTable = ({ selectedRow, onOpenChange }: { selectedRow: WorkflowDataRow; onOpenChange: (open: boolean) => void }) => {
  const [searchQuery, setSearchQuery] = useState("");
  const [toggledIds, setToggledIds] = useState<Set<string>>(new Set());
  const { data: users = [], isLoading } = useGetEntityUsers(selectedRow?.id);
  const { mutate } = useUpdateUserInWorkflowsPermissionsBatch();
  const { toast } = useToast();

  /** Toggles the active state of a user by tracking its id in a set */
  const toggleItem = useCallback((user: UserPerWorkflow) => {
    setToggledIds((prev) => {
...
export default WorkflowsSheetTable;
