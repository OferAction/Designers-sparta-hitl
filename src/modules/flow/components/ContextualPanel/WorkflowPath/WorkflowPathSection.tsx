...
import { TreeNode } from "@/modules/flow/components/ContextualPanel/WorkflowPath/TreeNode";
import { Node } from "@/modules/flow/types";

const selector = (state: ReactFlowState) => state.nodes as Node[];

export const WorkflowPathSection = () => {
  ...
  if (orderedNodes.length === 0) {
    return (
      <div className="px-4 py-2">
        <span className="text-muted-foreground font-inter text-sm">No workflow path available</span>
      </div>
    );
  }
