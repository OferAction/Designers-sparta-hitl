...
interface BranchOperationLoadingProps {
  mutationKeyFn: (queryClient: QueryClient) => { mutationKey: unknown[] };
  toastTitle: string;
  toastDescription?: React.ReactNode;
  containerId?: string;
}

export const BranchOperationLoading: React.FC<BranchOperationLoadingProps> = ({
  mutationKeyFn,
  toastTitle,
  toastDescription,
  ...
  return (
    <Portal.Root {...(container && { container })}>
      <div className={`flex-grow z-50 absolute flex items-center justify-center bg-muted/40 ${container ? "size-full" : "inset-0"}`} />
    </Portal.Root>
  );
};
