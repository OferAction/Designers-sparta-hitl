...
type NodesCatalogPluginsProps = {
  ...
  onSelectPlugin: (nodeType: string) => void;
};

export const ConnectorsCatalogList: React.FC<NodesCatalogPluginsProps> = ({
  commandListRef,
  searchValue,
  selectedNodeType,
  nodeTemplates,
  ...
  if (isNodesLoading) {
    return <div className="w-full mt-2 flex items-center justify-center text-primary">Loading connectors...</div>;
  }
  if (isNodesError) {
    return <div className="w-full mt-2 flex items-center justify-center text-destructive">Error loading connectors</div>;
  }

  return (
    <CommandList ref={commandListRef} className="max-h-full scrollbar-none">
      {filteredAndSortedItems.length === 0 && searchValue.trim() !== "" ? (
        <div className="py-6 text-center text-muted-foreground">No Connectors Found</div>
      ) : (
        <>
            ...
            <div>
              <CollapsibleTrigger asChild>
                <button className="flex items-center justify-between w-full py-2 pr-2">
                  <span className="text-xs text-sidebar-foreground">Connectors</span>
                  <CaretDownIcon
              ...
              <CollapsibleContent>
                {filteredAndSortedItems.map((node: any) => {
                        ...
                        <Tooltip>
                          <TooltipTrigger className="w-full">
                            <CommandItem
                              ...
                              onSelect={() => onSelectPlugin(node.data.name)}
                              draggable
                              tabIndex={0}
